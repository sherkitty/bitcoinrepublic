#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Bitcoin Republic Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing bitcoinrepublic-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='bitcoinrepublic-cli', help='bitcoinrepublic-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='BitcoinRepublic address to which the faucet should send')
...
parser.add_argument('bitcoinrepublic_cli_args', nargs='*', help='Arguments to pass on to bitcoinrepublic-cli (default: -signet)')

args = parser.parse_args()

if args.bitcoinrepublic_cli_args == []:
    args.bitcoinrepublic_cli_args = ['-signet']


def bitcoinrepublic_cli(rpc_command_and_params):
    argv = [args.cmd] + args.bitcoinrepublic_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = bitcoinrepublic_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = bitcoinrepublic_cli(['getnewaddress', 'faucet', 'bech32'])
