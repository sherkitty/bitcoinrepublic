# Bootstrappable BitcoinRepublic Core Builds

This directory contains the files necessary to perform bootstrappable BitcoinRepublic
Core builds.
...
To build BitcoinRepublic Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      BitcoinRepublic Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/bitcoinrepublic-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/sherkitty/bitcoinrepublic-guix.sigs).
