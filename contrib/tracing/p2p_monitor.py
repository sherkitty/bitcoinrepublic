#!/usr/bin/env python3
# Copyright (c) 2021 The Bitcoin Republic Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

""" Interactive bitcoinrepublicd P2P network traffic monitor utilizing USDT and the
    net:inbound_message and net:outbound_message tracepoints. """

# This script demonstrates what USDT for BitcoinRepublic Core can enable. It uses BCC
...
def main(bitcoinrepublicd_path):
    peers = dict()

    bitcoinrepublicd_with_usdts = USDT(path=str(bitcoinrepublicd_path))

    # attaching the trace functions defined in the BPF program to the tracepoints
    bitcoinrepublicd_with_usdts.enable_probe(
        probe="inbound_message", fn_name="trace_inbound_message")
    bitcoinrepublicd_with_usdts.enable_probe(
        probe="outbound_message", fn_name="trace_outbound_message")
    bpf = BPF(text=program, usdt_contexts=[bitcoinrepublicd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE:", sys.argv[0], "path/to/bitcoinrepublicd")
        exit()
    path = sys.argv[1]
    main(path)
