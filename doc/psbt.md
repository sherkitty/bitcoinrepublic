# PSBT Howto for BitcoinRepublic Core

Since BitcoinRepublic Core 0.17, an RPC interface exists for Partially Signed BitcoinRepublic
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/bitcoinrepublic/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for BitcoinRepublic transactions that are not fully signed
...
#### Multisig with multiple BitcoinRepublic Core instances
...
BitcoinRepublic Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `bitcoinrepublic-cli` is used.
...
Later, when *V* BTCREP has been received on *Amulti*, and Bob and Carol want to
