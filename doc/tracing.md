# User-space, Statically Defined Tracing (USDT) for BitcoinRepublic Core

BitcoinRepublic Core includes statically defined tracepoints to allow for more
...
======================
                ...
                ┌──────────────────┐            ┌──────────────┐
                │ tracing script   │            │ bitcoinrepublicd     │
                │==================│      2.    │==============│
                │  eBPF  │ tracing │      hooks │              │
...
**Note**: BitcoinRepublic Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to BitcoinRepublic Core
...
Multiple tools can list the available tracepoints in a `bitcoinrepublicd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in BitcoinRepublic Core, use `info probes` in `gdb`:

```
$ gdb ./src/bitcoinrepublicd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e /src/bitcoinrepublicd
stap net        outbound_message 0x0000000000107c05 /src/bitcoinrepublicd
stap validation block_connected  0x00000000002fb10c /src/bitcoinrepublicd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in BitcoinRepublic Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/bitcoinrepublicd | grep NT_STAPSDT -A 4 -B 2
...
```
$ tplist -l ./src/bitcoinrepublicd -v
