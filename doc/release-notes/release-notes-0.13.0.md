BitcoinRepublic Core version 0.13.0 is now available from:

  <https://bitcoinrepublic.org/bin/bitcoinrepublic-core-0.13.0/>
...
- Adding `dbcache=100` in bitcoinrepublic.conf
...
bitcoinrepublic-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/bitcoinrepublic-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Bitcoin Republic Core code base has
...
compatible peers.
[BIP 152](https://github.com/bitcoinrepublic/bips/blob/master/bip-0152.mediawiki)
...
HD wallets are incompatible with older versions of BitcoinRepublic Core.

[Pull request](https://github.com/sherkitty/bitcoinrepublic.git/8035/files), [BIP 32](https://github.com/bitcoinrepublic/bips/blob/master/bip-0032.mediawiki)
...
141](https://github.com/bitcoinrepublic/bips/blob/master/bip-0141.mediawiki), [BIP
143](https://github.com/bitcoinrepublic/bips/blob/master/bip-0143.mediawiki), [BIP
144](https://github.com/bitcoinrepublic/bips/blob/master/bip-0144.mediawiki), and [BIP
145](https://github.com/bitcoinrepublic/bips/blob/master/bip-0145.mediawiki) are
finished and included in this release.  However, BIP 141 does not yet specify
activation parameters on mainnet, and so this release does not support segwit
use on mainnet.  Testnet use is supported, and after BIP 141 is updated with
proposed parameters, a future release of BitcoinRepublic Core is expected that
...
Witness)] (https://github.com/bitcoinrepublic/bips/blob/master/bip-0141.mediawiki).
...
(https://github.com/bitcoinrepublic/bips/blob/master/bip-0141.mediawiki) for additional
details.)
...
The two phases are distinct in the BitcoinRepublic-Qt GUI. During the first one,
...
- The optional new p2p message "feefilter" is implemented and the protocol
  ...
  feerate. [BIP 133](https://github.com/bitcoinrepublic/bips/blob/master/bip-0133.mediawiki)
...
65](https://github.com/bitcoinrepublic/bips/blob/master/bip-0065.mediawiki)

  - OP_NOP3 has been renamed to OP_CHECKSEQUENCEVERIFY by [BIP 
112](https://github.com/bitcoinrepublic/bips/blob/master/bip-0112.mediawiki)
...
- Each ZMQ notification now contains an up-counting sequence number that allows
  ...
  PR [#7762](https://github.com/sherkitty/bitcoinrepublic.git/7762).
...
- #7550 `8b958ab` Input-from-stdin mode for bitcoinrepublic-cli (laanwj)
...
- #8153 `75ec320` `fundrawtransaction` feeRate: Use BTCREP/kB (MarcoFalke)
...
- #7613 `3798e5d` Add autocomplete to bitcoinrepublic-qt's console window (GamerSg)
...
- #7636 `fb0ac48` Add bitcoinrepublic address label to request payment QR code (makevoid)
- #8231 `760a6c7` Fix a bug where the SplashScreen will not be hidden during startup (jonasschnelli)
- #8256 `af2421c` BUG: bitcoinrepublic-qt crash (fsb4000)
- #8257 `ff03c50` Do not ask a UI question from bitcoinrepublicd (sipa)
- #8288 `91abb77` Network-specific example address (laanwj)
- #7707 `a914968` UI support for abandoned transactions (jonasschnelli)
- #8207 `f7a403b` Add a link to the BitcoinRepublic-Core repository and website to the About Dialog (MarcoFalke)
...
- #1099 `409a8a1` Revert fatal-ness of missing python-zmq (laanwj)
...
- #8047 `37f9a1f` Test_framework: Set wait-timeout for bitcoinrepublicd procs (MarcoFalke)
...
- #8164 `0f24eaf` [BitcoinRepublic-Tx] fix missing test fixtures, fix 32bit atoi issue (jonasschnelli)
...
- #7744 `d8e862a` test_framework: detect failure of bitcoinrepublicd startup (laanwj)
...
- #34731 `cd0910b` Fix p2p-feefilter.py for changed tx relay behavior (sdaftuar)
...
- #7975 `fc23fee` Update bitcoinrepublic-core GitHub links (MarcoFalke)
