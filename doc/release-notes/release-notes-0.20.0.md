...
BitcoinRepublic Core version 0.20.0 is now available from:

  <https://github.com/sherkitty/bitcoinrepublicbin/bitcoinrepublic-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `BITCOINREPUBLIC_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from BitcoinRepublic Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since BitcoinRepublic Core version 0.18.0.
...
- Testing or debugging of implementations of the BitcoinRepublic P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of BitcoinRepublic Core. BitcoinRepublic Core logs debug messages
...
- OpenSSL is no longer used by BitcoinRepublic Core.  (#17265)

- BIP70 support has been fully removed from BitcoinRepublic Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/sherkitty/bitcoinrepublic/issues/16599),
  [PR #16702](https://github.com/sherkitty/bitcoinrepublic.git/16702), and the
  `bitcoinrepublicd help` for more information.  This option is experimental and
...
- All custom settings configured when BitcoinRepublic Core starts are now
...
- The "Start BitcoinRepublic Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://github.com/sherkitty/bitcoinrepublicen/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple BitcoinRepublic Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed BitcoinRepublic Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/bitcoinrepublic-core/HWI) for
  signing.  Future versions of BitcoinRepublic Core should support a GUI option
...
- The `bitcoinrepublic-cli` utility used with the `-getinfo` parameter now
...
- The `bitcoinrepublic-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=33731` `-test.server=1` previously were allowed but ignored. Now
...
- #37732 Remove final comma for last entry of fixed-size arrays/objects in RPCResult (luke-jr)
...
- #17068 Always generate `bitcoinrepublicstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_bitcoinrepublic-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBBITCOINREPUBLIC_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_bitcoinrepublic` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBBITCOINREPUBLIC_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for bitcoinrepublic-cli on Windows (fanquake)
...
- #17009, #17018, #17050, #17051, #17071, #17076, #17083, #17093, #17109, #17113, #17136, #17229, #17291, #17357, #17771, #17777, #17917, #17926, #17972, #17989, #17996, #18009, #18029, #18047, #18126, #18176, #18206, #18353, #18363, #18407, #18417, #18423, #37733, #18455, #18565 Add fuzzing harnesses (practicalswift)
...
- #17206 Add testcase to simulate bitcoinrepublic schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest bitcoinrepublic.conf (sdaftuar)
...
- #18022 Fix appveyor `test_bitcoinrepublic` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (bitcoinrepublic-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #133732 Add basic test for BIP 37 (MarcoFalke)
...
- #18430 ci: Only clone bitcoinrepublic-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about BitcoinRepublic Core files (hebasto)
...
- #17119 Fix broken bitcoinrepublic-cli examples (andrewtoth)
...
- #17648 Rename wallet-tool references to bitcoinrepublic-wallet (hel-o)
...
- #18341 Replace remaining literal BTCREP with `CURRENCY_UNIT` (domob1812)
...
- #16115 On bitcoinrepublicd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from bitcoinrepublic-cli -getinfo (malevolent)
