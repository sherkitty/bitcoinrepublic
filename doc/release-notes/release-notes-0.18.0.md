BitcoinRepublic Core version 0.18.0 is now available from:

  <https://github.com/sherkitty/bitcoinrepublicbin/bitcoinrepublic-core-0.18.0/>
...
`/Applications/BitcoinRepublic-Qt` (on Mac) or `bitcoinrepublicd`/`bitcoinrepublic-qt` (on
Linux).
...
recommended to use BitcoinRepublic Core on unsupported systems.

BitcoinRepublic Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, BitcoinRepublic Core does not yet change appearance when
...
`contrib/init/bitcoinrepublicd.service`, it has been changed to use
`/var/lib/bitcoinrepublicd` as the data directory instead of
`~bitcoinrepublic/.bitcoinrepublic`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/bitcoinrepublicd` will
exist has enough space (check using `df -h /var/lib/bitcoinrepublicd`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:33730:33730` (this is an extra `:33730` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://github.com/sherkitty/bitcoinrepublicen/contact/) the BitcoinRepublic Core
...
The systemd init file (`contrib/init/bitcoinrepublicd.service`) has been changed
to use `/var/lib/bitcoinrepublicd` as the data directory instead of
`~bitcoinrepublic/.bitcoinrepublic`. This change makes BitcoinRepublic Core more consistent with
...
`datadir` option in `/etc/bitcoinrepublic/bitcoinrepublic.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/bitcoinrepublic/bitcoinrepublic.conf`.


Documentation
-------------

- A new short [document](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/bitcoinrepublic-conf.md)
  about the `bitcoinrepublic.conf` file describes how to use it to configure
  BitcoinRepublic Core.

- A new document introduces BitcoinRepublic Core's BIP174 [Partially-Signed
  BitcoinRepublic Transactions
  (PSBT)](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent BitcoinRepublic-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  BitcoinRepublic Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/sherkitty/bitcoinrepublic/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when BitcoinRepublic Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of BitcoinRepublic Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart bitcoinrepublicd with the `-deprecatedrpc=generate`
...
- A new `bitcoinrepublic-wallet` tool is now distributed alongside BitcoinRepublic
...
This section describes planned changes to BitcoinRepublic Core that may affect
other BitcoinRepublic software and services.

- Since version 0.16.0, BitcoinRepublic Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with BitcoinRepublic Core 0.20 (expected about
  a year after 0.18), BitcoinRepublic Core will default to native segwit
  ...
  to bech32 addresses, and if the BitcoinRepublic Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in BitcoinRepublic Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any BitcoinRepublic Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, BitcoinRepublic Core
...
- A sub-project of BitcoinRepublic Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with BitcoinRepublic Core.  See their [project
  page](https://github.com/bitcoinrepublic-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to BitcoinRepublic Core's own implementation, although entropy
  gathered by BitcoinRepublic Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves BitcoinRepublic Core a
...
- On macOS, BitcoinRepublic Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `bitcoinrepublic_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libbitcoinrepublic_server.a` (jnewbery)
...
- #13926 [Tools] bitcoinrepublic-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like bitcoinrepublicd (MarcoFalke)
...
- #14521 qt, docs: Fix `bitcoinrepublic-qt -version` output formatting (hebasto)
...
- #11625 Add BitcoinRepublicApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libbitcoinrepublic_server` (laanwj)
...
- #15154 Configure: bitcoinrepublic-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/bitcoinrepublic-wallet (Sjors)
...
- #11526 Visual Studio build configuration for BitcoinRepublic Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libbitcoinrepublic_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in bitcoinrepublic-util-test.py (isghe)
...
- #12775 Integration of property based testing into BitcoinRepublic Core (Christewart)
...
- #12246 Bugfix: Only run bitcoinrepublic-tx tests when bitcoinrepublic-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_bitcoinrepublic` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_bitcoinrepublic` (MarcoFalke)
...
- #14186 bitcoinrepublic-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_bitcoinrepublic.conf` message on startup if conf file exists (leishman)
...
- #12255 Update bitcoinrepublic.service to conform to init.md (dongcarl)
...
- #14497 Add doc/bitcoinrepublic-conf.md (hebasto)
...
- #15354 Add missing `bitcoinrepublic-wallet` tool manpages (MarcoFalke)
...
- #15765 correct bitcoinrepublicconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support bitcoinrepublic folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/bitcoinrepublic/).
