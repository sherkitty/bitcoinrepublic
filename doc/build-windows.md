...
Below are some notes on how to build BitcoinRepublic Core for Windows.

The options known to work for building BitcoinRepublic Core on Windows are:
...
Note that for WSL the BitcoinRepublic Core source path MUST be somewhere in the default mount file system, for
example /usr/src/bitcoinrepublic, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\bitcoinrepublic`, for example:

    make install DESTDIR=/mnt/c/workspace/bitcoinrepublic
