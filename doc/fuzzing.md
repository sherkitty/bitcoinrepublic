# Fuzzing BitcoinRepublic Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing BitcoinRepublic Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/sherkitty/bitcoinrepublic
$ cd bitcoinrepublic/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
## Overview of BitcoinRepublic Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/bitcoinrepublic_report.pdf) includes an external overview of the status of BitcoinRepublic Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/sherkitty/bitcoinrepublic/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/sherkitty/bitcoinrepublic/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/sherkitty/bitcoinrepublic/tree/master/src/test/fuzz).
...
It is possible to specify `bitcoinrepublicd` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`bitcoinrepublic-core/qa-assets`](https://github.com/bitcoinrepublic-core/qa-assets) repo.

To fuzz `process_message` using the [`bitcoinrepublic-core/qa-assets`](https://github.com/bitcoinrepublic-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/bitcoinrepublic-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`bitcoinrepublic-core/qa-assets`](https://github.com/bitcoinrepublic-core/qa-assets) repo.

Every single pull request submitted against the BitcoinRepublic Core repo is automatically tested against all inputs in the [`bitcoinrepublic-core/qa-assets`](https://github.com/bitcoinrepublic-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make BitcoinRepublic Core more robust.
...
with certain assembly code from BitcoinRepublic Core's code. See [developer notes on sanitizers](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing BitcoinRepublic Core using afl++

## Quickstart guide

To quickly get started fuzzing BitcoinRepublic Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing BitcoinRepublic Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing BitcoinRepublic Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the BitcoinRepublic Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as BitcoinRepublic
Core without having to write any custom fuzzing harness. The `bitcoinrepublicd` server
...
```sh
$ mkdir bitcoinrepublic-honggfuzz-p2p/
$ cd bitcoinrepublic-honggfuzz-p2p/
$ git clone https://github.com/sherkitty/bitcoinrepublic
$ cd bitcoinrepublic/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/bitcoinrepublicd.cpp b/src/bitcoinrepublicd.cpp
index 455a82e39..2faa3f80f 100644
--- a/src/bitcoinrepublicd.cpp
+++ b/src/bitcoinrepublicd.cpp
...
EOF
$ make -C src/ bitcoinrepublicd
$ mkdir -p inputs/
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=37732 -f inputs/ -- \
          src/bitcoinrepublicd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
                       -nodebuglogfile -bind=127.0.0.1:37732 -logthreadnames \
                       -debug
```

# Fuzzing BitcoinRepublic Core using Eclipser (v1.x)

## Quickstart guide

To quickly get started fuzzing BitcoinRepublic Core using [Eclipser v1.x](https://github.com/SoftSec-KAIST/Eclipser/tree/v1.x):
...
See [this proposed patch](https://github.com/sherkitty/bitcoinrepublic.git/22472) for more information.
...
BitcoinRepublic Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/bitcoinrepublic-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=bitcoinrepublic-core).
...
because BitcoinRepublic is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_bitcoinrepublic-core/latest).
