# CJDNS support in BitcoinRepublic Core

It is possible to run BitcoinRepublic Core over CJDNS, an encrypted IPv6 network that
...
done outside of BitcoinRepublic Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the BitcoinRepublic network and individual nodes.
...
before it will work with your BitcoinRepublic Core node. This is described in steps
...
## Run BitcoinRepublic Core with CJDNS

Once you are connected to the CJDNS network, the following BitcoinRepublic Core
...
When enabled, this option tells BitcoinRepublic Core that it is running in an
...
IPv6 local network. This helps BitcoinRepublic Core perform better address management:
...
CJDNS support was added to BitcoinRepublic Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `bitcoinrepublic-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/sherkitty/bitcoinrepublic/blob/master/doc/tor.md) for
details.

## CJDNS-related information in BitcoinRepublic Core

There are several ways to see your CJDNS address in BitcoinRepublic Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `bitcoinrepublic-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `bitcoinrepublic-cli getpeerinfo`).
