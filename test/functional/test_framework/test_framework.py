#!/usr/bin/env python3
# Copyright (c) 2014-2022 The Bitcoin Republic Core developers
...
TMPDIR_PREFIX = "bitcoinrepublic_func_test_"
...
class BitcoinRepublicTestMetaClass(type):
    """Metaclass for BitcoinRepublicTestFramework.

    Ensures that any attempt to register a subclass of `BitcoinRepublicTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'BitcoinRepublicTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("BitcoinRepublicTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("BitcoinRepublicTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class BitcoinRepublicTestFramework(metaclass=BitcoinRepublicTestMetaClass):
    """Base class for a bitcoinrepublic test script.

    Individual bitcoinrepublic test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave bitcoinrepublicds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop bitcoinrepublicds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use bitcoinrepublic-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_bitcoinrepublicd = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinrepublicd" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinrepubliccli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinrepublic-cli" + config["environment"]["EXEEXT"],
        )
        fname_bitcoinrepublicutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "bitcoinrepublic-util" + config["environment"]["EXEEXT"],
        )
        self.options.bitcoinrepublicd = os.getenv("BITCOINREPUBLICREPUBLICD", default=fname_bitcoinrepublicd)
        self.options.bitcoinrepubliccli = os.getenv("BITCOINREPUBLICCLI", default=fname_bitcoinrepubliccli)
        self.options.bitcoinrepublicutil = os.getenv("BITCOINREPUBLICUTIL", default=fname_bitcoinrepublicutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: bitcoinrepublicds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("BitcoinRepublicRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'bitcoinrepublicd', self.options.bitcoinrepublicd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'bitcoinrepublic-cli', self.options.bitcoinrepubliccli) for v in versions]
        ...
        for i in range(num_nodes):
            test_node_i = TestNode(
                ...
                bitcoinrepublicd=binary[i],
                bitcoinrepublic_cli=binary_cli[i],
                version=versions[i],
    ...
    def start_nodes(self, extra_args=None, *args, **kwargs):
        """Start multiple bitcoinrepublicds"""
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a bitcoinrepublicd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple bitcoinrepublicd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as bitcoinrepublicd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    bitcoinrepublicd=self.options.bitcoinrepublicd,
                    bitcoinrepublic_cli=self.options.bitcoinrepubliccli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in bitcoinrepublic.conf
    ...
    def skip_if_no_bitcoinrepublicd_tracepoints(self):
        """Skip the running test if bitcoinrepublicd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("bitcoinrepublicd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_bitcoinrepublicd_zmq(self):
        """Skip the running test if bitcoinrepublicd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("bitcoinrepublicd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if bitcoinrepublic-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("bitcoinrepublic-wallet has not been compiled")

    def skip_if_no_bitcoinrepublic_util(self):
        """Skip the running test if bitcoinrepublic-util has not been compiled."""
        if not self.is_bitcoinrepublic_util_compiled():
            raise SkipTest("bitcoinrepublic-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if bitcoinrepublic-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("bitcoinrepublic-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether bitcoinrepublic-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether bitcoinrepublic-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_bitcoinrepublic_util_compiled(self):
        """Checks whether bitcoinrepublic-util was compiled."""
        return self.config["components"].getboolean("ENABLE_BITCOINREPUBLIC_UTIL")
