This directory contains integration tests that test bitcoinrepublicd and its
...
bitcoinrepublicd and bitcoinrepublic-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (bitcoinrepublic-util, bitcoinrepublic-tx, ...).
...
Before tests can be run locally, BitcoinRepublic Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the bitcoinrepublicd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another bitcoinrepublicd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its bitcoinrepublicd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other bitcoinrepublicd processes are running.

On linux, the test framework will warn if there is another
bitcoinrepublicd process running when the tests are started.

If there are zombie bitcoinrepublicd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
bitcoinrepublicd processes running on the system, so should not be used if any non-test
bitcoinrepublicd processes are being run.**

```bash
killall bitcoinrepublicd
```

or

```bash
pkill -9 bitcoinrepublicd
```
...
sure bitcoinrepublicd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and bitcoinrepublicd `debug.log`s will all be dumped
...
`test_framework.log` and bitcoinrepublicd `debug.log`s can be combined into a single
...
call methods that interact with the bitcoinrepublicd nodes-under-test.

If further introspection of the bitcoinrepublicd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/bitcoinrepublicd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/bitcoinrepublicd <pid>
```
