...
const char * const BITCOINREPUBLIC_CONF_FILENAME = "bitcoinrepublic.conf";
const char * const BITCOINREPUBLIC_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //bitcoinrepublic-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", BITCOINREPUBLIC_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\BitcoinRepublic
    // macOS: ~/Library/Application Support/BitcoinRepublic
    // Unix-like: ~/.bitcoinrepublic
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitcoinRepublic";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/BitcoinRepublic";
#else
    // Unix-like
    return pathRet / ".bitcoinrepublic";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", BITCOINREPUBLIC_CONF_FILENAME));
}
