...
#include <qt/bitcoinrepublicaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BitcoinRepublic address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new BitcoinRepublicAddressEntryValidator(parent));
    widget->setCheckValidator(new BitcoinRepublicAddressCheckValidator(parent));
}
...
bool parseBitcoinRepublicURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no bitcoinrepublic: URI
    if(!uri.isValid() || uri.scheme() != QString("bitcoinrepublic"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!BitcoinRepublicUnits::parse(BitcoinRepublicUnit::BTCREP, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseBitcoinRepublicURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseBitcoinRepublicURI(uriInstance, out);
}

QString formatBitcoinRepublicURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("bitcoinrepublic:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinRepublicUnits::format(BitcoinRepublicUnit::BTCREP, info.amount, false, BitcoinRepublicUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openBitcoinRepublicConf()
{
    ...
    /* Open bitcoinrepublic.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinRepublic.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "BitcoinRepublic (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("BitcoinRepublic (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for BitcoinRepublic*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "bitcoinrepublic.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("bitcoinrepublic-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a bitcoinrepublic.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=BitcoinRepublic\n";
        else
            optionFile << strprintf("Name=BitcoinRepublic (%s)\n", chain);
