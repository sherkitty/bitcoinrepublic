This directory contains the source code for the BitcoinRepublic Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
#### bitcoinrepublicgui.(h/cpp)

- Represents the main window of the BitcoinRepublic UI.
...
- ClientModel is used by the main application `bitcoinrepublicgui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a bitcoinrepublic:... link from a browser).
...
* UI elements like BitcoinRepublicAmountField, which inherit from QWidget.
* `bitcoinrepublicstrings.cpp`: automatically generated
* `bitcoinrepublicunits.(h/cpp)`: BTCREP / mBTCREP / etc. handling
* `callback.h`
...
**Note:** Do not change `local/bitcoinrepublic_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `bitcoinrepublic-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `bitcoinrepublic-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `bitcoinrepublic-qt`
9. You're all set! Start developing, building, and debugging the BitcoinRepublic Core GUI
