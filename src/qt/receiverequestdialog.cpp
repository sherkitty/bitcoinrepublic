...
#include <config/bitcoinrepublic-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::setInfo(const SendCoinsRecipient &_info)
{
    ...
    QString uri = GUIUtil::formatBitcoinRepublicURI(info);
...
{
    if (!model) return;
    ui->amount_content->setText(BitcoinRepublicUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount));
}

void ReceiveRequestDialog::on_btnCopyURI_clicked()
{
    GUIUtil::setClipboard(GUIUtil::formatBitcoinRepublicURI(info));
}
