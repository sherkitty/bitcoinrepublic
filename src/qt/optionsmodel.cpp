...
/** Call node.updateRwSetting() with BitcoinRepublic 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because BitcoinRepublic 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/sherkitty/bitcoinrepublic.git/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because bitcoinrepublic 22.x will not try to read these.
...
//! Convert enabled/size values to bitcoinrepublic -prune setting.
...
//! Get pruning enabled value to show in GUI from bitcoinrepublic -prune setting.
...
//! Get pruning size value to show in GUI from bitcoinrepublic -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayBitcoinRepublicUnit")) {
        settings.setValue("DisplayBitcoinRepublicUnit", QVariant::fromValue(BitcoinRepublicUnit::BTCREP));
    }
    QVariant unit = settings.value("DisplayBitcoinRepublicUnit");
    if (unit.canConvert<BitcoinRepublicUnit>()) {
        m_display_bitcoinrepublic_unit = unit.value<BitcoinRepublicUnit>();
    } else {
        m_display_bitcoinrepublic_unit = BitcoinRepublicUnit::BTCREP;
        settings.setValue("DisplayBitcoinRepublicUnit", QVariant::fromValue(m_display_bitcoinrepublic_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if bitcoinrepublic.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_bitcoinrepublic_unit);
...
{
    if (new_unit.isNull() || new_unit.value<BitcoinRepublicUnit>() == m_display_bitcoinrepublic_unit) return;
    m_display_bitcoinrepublic_unit = new_unit.value<BitcoinRepublicUnit>();
    QSettings settings;
    settings.setValue("DisplayBitcoinRepublicUnit", QVariant::fromValue(m_display_bitcoinrepublic_unit));
    Q_EMIT displayUnitChanged(m_display_bitcoinrepublic_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/sherkitty/bitcoinrepublic.git/8273
    ...
    // (https://github.com/sherkitty/bitcoinrepublic-gui/issues/567).
