...
static void SetupBitcoinRepublicTxArgs(ArgsManager &argsman)
{
...
{
    SetupBitcoinRepublicTxArgs(gArgs);
    std::string error;
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " bitcoinrepublic-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.IsArgSet("-version")) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "Usage:  bitcoinrepublic-tx [options] <hex-tx> [commands]  Update hex-encoded bitcoinrepublic transaction\n"
                "or:     bitcoinrepublic-tx [options] -create [commands]   Create hex-encoded bitcoinrepublic transaction\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitcoinrepublic transaction
